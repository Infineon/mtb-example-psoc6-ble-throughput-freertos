...
uint16_t              fullUpdateTime;
uint16_t              partialUpdateTime;

/* Pointers for the data structures used by the driver */
uint8_t*              dataLineEven;
uint8_t*              dataLineOdd;
uint8_t*              dataLineScan;

/* Scan table for the four stages of display update*/
uint8_t const        scanTable[PV_EINK_SCAN_TABLE_SIZE] = PV_EINK_SCAN_TABLE_DATA;


/* The SPI register addresses for Channel Select */
uint8_t const        channelSelect[PV_EINK_CHANNEL_SEL_SIZE] = PV_EINK_CHANNEL_SEL_DATA;
