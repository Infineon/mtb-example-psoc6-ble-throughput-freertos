...
static const char target_name[] = "TPUT";
    ...
    switch(event)
    {
        ...
        ***********************************************************************/
        ...
        case CY_BLE_EVT_GAP_DEVICE_CONNECTED:
        {
            ...
            phy_pram.rxPhyMask = CY_BLE_PHY_MASK_LE_2M;
            phy_pram.txPhyMask = CY_BLE_PHY_MASK_LE_2M;
        ...
        ***********************************************************************/
        ...
        case CY_BLE_EVT_GATTC_XCHNG_MTU_RSP:
        {
            ...
            if(ble_api_result == CY_BLE_SUCCESS)
            {
                iprintf("Discover Characteristic Descriptor API success");
            }
            else
            {
                eprintf("Discover Characteristic Descriptor API, errorcode = 0x%X", ble_api_result);
            }
            break;
        }
...
*
***************************************************************************************************************************/
static uint8* adv_parser(uint16_t AD_type, cy_stc_ble_gapc_adv_report_param_t*
                         scan_report, uint8* adv_type_length)
{
    uint8 length =0u;
    uint8* pName = NULL;

    for(uint8 i=0u; i<scan_report->dataLen; i+=length+1)
    {
        length = scan_report->data[i];
        if(scan_report->data[i+1] == AD_type)
        {
